...
module.exports.containsOnly = function (array, value) {
  if (array !== null && array.length === 1 && array[0] === value) {
    return true;
  }
  return false;
};
...
module.exports.createAndroidLocalProperties = function (path) {
  if (!fs.existsSync(`${path}/android/local.properties`)) {
    let androidHome = process.env.ANDROID_HOME;
    if (androidHome) {
      let sdkDir = 'sdk.dir=' + androidHome;
      fs.writeFile(`${path}/android/local.properties`, sdkDir, function (error) {
        console.error(error);
      });
