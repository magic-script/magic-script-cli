...
function renameComponentsFiles (folderName, packageName, visibleName) {
  ...
  if (!visibleName) {
    ...
    var osDependendCommandPath;
    if (process.platform === 'win32') {
      osDependendCommandPath = 'node node_modules/react-native-rename/lib/index.js';
    } else {
      osDependendCommandPath = 'node_modules/react-native-rename/lib/index.js';
    }
    var renamingToolPath = path.join(
      __dirname,
      '..',
      osDependendCommandPath
    );
